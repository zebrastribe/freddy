...
export async function getTraceStatus() {
  try {
    const db = getDb();
    const statusDoc = await getDoc(doc(db, "trace_status", "current"));
...
export async function setTraceStatus(mode) {
  try {
    const db = getDb();
    await setDoc(doc(db, "trace_status", "current"), { mode });
  } catch (error) {
    console.error('Failed to set Trace status:', error);
  }
}
