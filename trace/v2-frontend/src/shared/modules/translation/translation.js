...
export class Translation {
  ...
  setLanguage(language) {
    ...
    const supportedLanguages = ['da_DK', 'en_GB'];
    if (supportedLanguages.includes(language)) {
      this.language = language;
      console.log('🌐 Language set to:', language);
      return true;
    } else {
      console.warn('🌐 Unsupported language:', language, 'using browser detection instead');
      return false;
    }
  }
  ...
  async loadTranslations() {
    try {
      const response = await fetch(`/src/shared/modules/translation/json/${this.language}.json`);
      this.translations = await response.json();
      console.log('🌐 Loaded translations for:', this.language);
      console.log('🌐 Translation keys loaded:', Object.keys(this.translations));
    } catch (error) {
