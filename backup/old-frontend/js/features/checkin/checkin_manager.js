...
class CheckInManager {
  ...
  /**
   * Get check-ins for a specific pet (public access)
   * @param {string} petId - Pet ID
   * @param {number} limitCount - Maximum number of check-ins to return
   * @returns {Promise<Array>} Array of check-in documents
   */
  async getPetCheckIns(petId, limitCount = 50) {
    try {
      console.log('getPetCheckIns: Getting public check-ins for pet:', petId);
      
      // Use direct Firestore query with filtering for better performance
      ...
      console.log('getPetCheckIns: Found', checkins.length, 'check-ins for pet', petId);
   ...
   * @returns {Promise<Array>} Array of check-in documents
   */
  async getPublicCheckIns(limitCount = 100) {
    try {
  ...
  async getCheckInStats(petId = null, userId = null) {
    try {
      ...
      if (petId) {
        // Pet-specific stats - public access
        console.log('getCheckInStats: Getting public stats for pet:', petId);
      ...
      checkins.forEach(checkin => {
        ...
        // Count by date
        const date = new Date(checkin.timestamp?.toDate ? checkin.timestamp.toDate() : checkin.timestamp).toDateString();
        stats.byDate[date] = (stats.byDate[date] || 0) + 1;

        // Count by hour
        const hour = new Date(checkin.timestamp?.toDate ? checkin.timestamp.toDate() : checkin.timestamp).getHours();
  ...
  async searchCheckIns(criteria = {}) {
    try {
      ...
      // Apply filters
      if (criteria.petId) {
        // Pet-specific searches - public access
        console.log('searchCheckIns: Public search for pet:', criteria.petId);
      ...
      if (criteria.startDate) {
        checkins = checkins.filter(checkin => {
          ...
          return timestamp >= new Date(criteria.startDate);
        });
      }
      ...
      if (criteria.endDate) {
        checkins = checkins.filter(checkin => {
          const timestamp = checkin.timestamp?.toDate ? checkin.timestamp.toDate() : new Date(checkin.timestamp);
          return timestamp <= new Date(criteria.endDate);
        });
      }
  ...
  async createLegacyCheckIn(checkinData) {
    try {
      ...
      const checkin = {
        id: checkinId,
        petId: checkinData.petId || null, // Use provided petId or null for backward compatibility
        userId: userId,
        name: checkinData.name || 'Anonymous',
        latitude: checkinData.latitude,
        longitude: checkinData.longitude,
